...
class Core_Topup extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        $grid = new Grid(new Topup());
        $grid->column('trans_ref', __('Mã giao dịch'));
        $grid->column('branch.branch_name', __('Chi nhánh'));
        $grid->column('account_id', __('Tài khoản nộp tiền'))->display(function ($accountNumber) {
            return UtilsCommonHelper::bankAccountGridFormatter($accountNumber);
        });
        $grid->column('amount', __('Số tiền nộp'))->display(function ($amount) {
            return ConstantHelper::moneyFormatter($amount);
        });
        $grid->column('value_date', __('Ngày nộp'))->display(function ($valueDate) {
    ...
    protected function detail($id)
    {
        ...
        $show = new Show(Topup::findOrFail($id));
        $show->field('trans_ref', __('Mã giao dịch'));
        $show->field('branch.branch_name', __('Chi nhánh'));
        $show->field('account_id', __('Tài khoản nộp tiền'))->display(function ($accountNumber) {
            return UtilsCommonHelper::bankAccountDetailFormatter($accountNumber);
        });
        $show->field('amount', __('Số tiền nộp'))->display(function ($amount) {
            return ConstantHelper::moneyFormatter($amount);
        });
        $show->field('value_date', __('Ngày nộp'))->display(function ($valueDate) {
            return ConstantHelper::dateFormatter($valueDate);
        });
    ...
    protected function form()
    {
        ...
        $form = new Form(new Topup());
        if ($form->isEditing()) {
            $id = request()->route()->parameter("topup");
            ...
            $form->select('account_id', __('Tài khoản nộp tiền'))->options($bankAccountOptions)->default($accountId)->readonly();
            $form->currency('amount', __('Số tiền nộp'))->symbol('VND')->readonly();
            $form->date('value_date', __('Ngày nộp'))->readonly();
            ...
            $tranferId = (new UtilsCommonHelper)->generateTransactionId("TP");
            ...
            $form->select('account_id', __('Tài khoản nộp tiền'))->options($bankAccountOptions);
            $form->currency('amount', __('Số tiền nộp'))->symbol('VND');
            $form->date('value_date', __('Ngày nộp'));
